/**
   utilities-rand-mod.c

   Utility functions across the areas of randomness, modular arithmetic, 
   and binary representation. Includes assert statements for debugging
   purposes that can be turned off with NDEBUG.
*/
...
bool bern_uint64(uint64_t threshold, uint64_t low, uint64_t high){
  ...
  uint64_t denom = 0xffffffffffffffff;
  long double p, f;
  ...
  p = (threshold - low) / (long double)(high - low);
  f = rand_n / (long double)denom;
  return f < p;
}
...
bool bern_uint32(uint32_t threshold, uint32_t low, uint32_t high){
  ...
  p = (threshold - low) / (double)(high - low);
  f = rand_n / (double)denom;
  return f < p;
}
...
uint64_t mul_mod_uint64(uint64_t a, uint64_t b, uint64_t n){
  ...
  uint64_t ah_bh, ah_bl, al_bh, al_bl;
  uint64_t ret;
  assert(n > 0);
...
*/
uint64_t mem_mod_uint64(const void *s, uint64_t size, uint64_t n){
  uint8_t *ptr;
...
/**
   ...
   of mem_mod_uint64.
*/
uint64_t fast_mem_mod_uint64(const void *s, uint64_t size, uint64_t n){
...
/**
   ...
   Does not require a little-endian machine.
*/
uint32_t mem_mod_uint32(const void *s, uint64_t size, uint32_t n){
  uint8_t *ptr;
  uint64_t n64 = n;
  uint64_t byte_val, prod;
  uint64_t pow_two = 1;
...
/**
   ...
   of mem_mod_uint32.
*/
uint32_t fast_mem_mod_uint32(const void *s, uint64_t size, uint32_t n){
  uint64_t step_size = sizeof(uint64_t);
  uint64_t *ptr;
  uint64_t n64 = n;
  uint64_t bytes_val, prod;
