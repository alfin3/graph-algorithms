...
/**
   ...
   as elt in ht_div_insert and the element is fully copied into the
...
/**
   Runs a ht_div_{remove, delete} test on distinct keys and uint64_t
...
/**
   ...
   pointer to an element is passed as elt in ht_div_insert, and
...
void run_corner_cases_test(){
  ...
  void *key = NULL;
  ht_div_t ht;
  ht_div_init(&ht, key_size, sizeof(uint64_t), alpha, NULL);
  ...
  res *= (ht.num_elts == 1);
  res *= (*(uint64_t *)ht_div_search(&ht, key) == elt);
