...
/**
   ...
   as elt in ht_mul_insert and the element is fully copied into the
...
/**
   Runs a ht_mul_{remove, delete} test on distinct keys and uint64_t
...
/**
   ...
   pointer to an element is passed as elt in ht_mul_insert, and
...
void run_corner_cases_test(){
  ...
  for (i = 0; i < num_inserts; i++){
    elt = i;
    ht_mul_insert(&ht, &key_a, &elt);
  }
  res *= (ht.count == ht_count);
  res *= (ht.num_elts == 1);
  res *= (*(uint64_t *)ht_mul_search(&ht, &key_a) == elt);
  res *= (ht_mul_search(&ht, &key_b) == NULL);
  ht_mul_insert(&ht, &key_b, &elt);
  res *= (ht.count == ht_count);
  res *= (ht.num_elts == 2);
  res *= (*(uint64_t *)ht_mul_search(&ht, &key_a) == elt);
  res *= (*(uint64_t *)ht_mul_search(&ht, &key_b) == elt);
  ht_mul_delete(&ht, &key_a);
  res *= (ht.count == ht_count);
  res *= (ht.num_elts == 1);
  res *= (ht_mul_search(&ht, &key_a) == NULL);
  res *= (*(uint64_t *)ht_mul_search(&ht, &key_b) == elt);
  ht_mul_delete(&ht, &key_b);
  res *= (ht.count == ht_count);
  res *= (ht.num_elts == 0);
  res *= (ht_mul_search(&ht, &key_a) == NULL);
  res *= (ht_mul_search(&ht, &key_b) == NULL);
